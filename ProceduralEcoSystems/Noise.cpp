...
glm::vec2 Noise::EcosystemGetHeightAndGradients(float x, float z, float normalOffset, float amplitude) {
    float hL = get(x, z - normalOffset) * amplitude;
    float hR = get(x, z + normalOffset) * amplitude;
    float hD = get(x - normalOffset, z) * amplitude;
    float hU = get(x + normalOffset, z) * amplitude;
    ...
    return glm::vec2(height, normal.y);
}
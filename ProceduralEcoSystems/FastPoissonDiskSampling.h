...
namespace thinks {
    ...
    auto PoissonDiskSampling(
        ...
        // Validate input.
        if (!(radius > FloatT{ 0 }) || !(max_sample_attempts > 0) || !pds::ValidBounds(x_min, x_max)) {
        ...
        pds::AddSample<VecTraitsT>(
            pds::VecRangeRand<VecT, VecTraitsT>(x_min, x_max, &local_seed),
            &samples,
            &active_indices,
            &grid
        );
        ...
        while (!active_indices.empty()) {
            // Randomly choose an active sample.
            ...
            while (attempt_count < max_sample_attempts) {
                ...
                if (pds::InsideBounds<VecTraitsT>(cand_sample, x_min, x_max)) {
                    ...
                    const auto grid_neighbors = pds::GridNeighborhood<VecTraitsT>(cand_sample, grid);
                    ...
                    if (!existing_sample) {
                        ...
                        pds::AddSample<VecTraitsT>(cand_sample, &samples, &active_indices, &grid);
                        break;
                    }
